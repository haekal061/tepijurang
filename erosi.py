...
class CitraDigitalApp:
    ...
    def toggle_erosion_buttons(self):
        if not self.erosi_mode:
            self.btn_erosi.grid_forget()
            self.btn_erosi_plus.grid(row=0, column=5, padx=3, pady=3)
            self.btn_erosi_diag.grid(row=0, column=6, padx=3, pady=3)
            self.erosi_mode = True
        else:
            self.btn_erosi_plus.grid_forget()
            self.btn_erosi_diag.grid_forget()
            self.btn_erosi.grid(row=0, column=5, padx=3, pady=3)
            self.erosi_mode = False

    def erosion(self, se_type):
        if self.img1 is not None:
            ...
            self.hasil = eroded
            self.display_image(eroded, self.panel_hasil, gray=True)
            self.clear_histogram()
        else:
            messagebox.showerror("Error", "Input Gambar 1 terlebih dahulu!")
        self.toggle_erosion_buttons()